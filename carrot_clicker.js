...
const default_player = {
    // game_version: false,
    data_version: 14, // needs to be incremented by 1 any time any game object is changed
...
const Default_Gregory = new Blacksmith(
    ...
    [0,0,0,0,0,0],
    [15000,750000,50000000,7000000000,500000000000,100000000000000],
    false
);
...
class tome {
    ...
    priceQuery(amount,sendNewPrice=false){
        if(typeof(amount)!="number") return console.error("amount needs to be a number");
        if(this.value+amount>this.max) return "∞";
        ...
        function query(){
            let scaling=1.001;
            if(valueDummy>50) scaling=1.02;
            if(valueDummy>125) scaling=1.01;
            if(valueDummy>375) scaling=1.003;
            if(valueDummy>1000) scaling=1.001
            if(valueDummy<=50)newPrice=Math.ceil(newPrice*scaling);
            if(valueDummy>50)newPrice=Math.floor(newPrice*scaling);
            if(target===valueDummy+1) return;
            sum+=newPrice;
            valueDummy+=1;
            query();
        }
...
function updateCharlesShop() {
    ...
    style(elCharles.shop.improveWorkingConditions, 'cant_afford', (Charles.improveWorkingConditions.price >= player.golden_carrots));
    style(elCharles.shop.betterHoes, 'cant_afford', (Charles.betterHoes.price >= player.golden_carrots));
    style(elCharles.shop.decreaseWages, 'cant_afford', (Charles.decreaseWages.price >= player.golden_carrots));
    ...
    // Update tome effects
    eInnerText(tomeEffect.iwc, `+${110 * Charles.improveWorkingConditions.value}%`);
...
function Calculate_Carrots(character) {
    let bH=1+Charles.betterHoes.value/100 // Calculate betterHoes bonus
    let Zcheck2 = Charles.improveWorkingConditions.value > 0 ? 0:1; // Zcheck Checks to see if result in 0 Carrots Per
    ...
    let iwc = Charles.improveWorkingConditions.value;
    let boosted = 1;
